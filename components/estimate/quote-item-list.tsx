...
export function QuoteItemList({ items, loading, onItemChange, onRemove }: QuoteItemListProps) {
  return (
                ...
                {item.is_manual ? (
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      <select
                        value={item.category}
                        onChange={(e) => onItemChange(item.client_id, { category: e.target.value })}
                      ...
                      <Input
                        ...
                        onChange={(e) => onItemChange(item.client_id, { customer_name: e.target.value })}
                        disabled={loading}
                      />
                    </div>
                    ...
                    {item.category === "직접입력" && (
                      <Input
                        ...
                        onChange={(e) => onItemChange(item.client_id, { custom_category: e.target.value })}
                        disabled={loading}
                      />
                    )}
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      <Input
                        ...
                        onChange={(e) => onItemChange(item.client_id, { internal_name: e.target.value })}
                        disabled={loading}
                      />
                      <select
                        value={item.unit}
                        onChange={(e) => onItemChange(item.client_id, { unit: e.target.value })}
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                        disabled={loading}
                      >
                    ...
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={(e) =>
                          onItemChange(item.client_id, { save_to_price_items: e.target.checked })
                        }
                        disabled={loading}
                      />
                      단가표에도 저장
